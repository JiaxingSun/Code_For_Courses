...
for n = 3:(3+totnum)
    fh = @(x)((eval_best_2norm_trap(fhandle,n,x,lower,upper,1000)-fhandle(x)).^2);
    err_2norm(n-2) = integral(fh,lower,upper,'AbsTol',1e-12,'ArrayValued',true);
end

for n = 3:(3+totnum)
    fh2 = @(x)(abs(eval_best_2norm_trap(fhandle,n,x,lower,upper,1000)-fhandle(x)));
...
xlabel('n');
ylabel('error');
title('Using Trapezoidal Rule for Integration');