...
st=0.05;%this is the step of the algorithm
...
plot(beta(:,[1,(numcyc-9):(numcyc)])),title('Estimated coefficients','FontSize',15);
...
svmlossl2=zeros(1,numcyc);%define the vector of objective function, dim=number of cycles, to see its evolution
for k=1:numcyc
    svmlossl2(1,k)=sum((labelsbi(:,1).*(images'*beta(:,(i-1)))<1).*(1-(labelsbi(:,1).*(images'*beta(:,(i-1))))))+lam*beta(:,k)'*beta(:,k);
end%calculate the vector of objective function
 plot(erate),title('Prediction error rates','FontSize',15),xlabel('number of iterates','FontSize',10);
 plot(svmlossl2),title('Evolution of loss function','FontSize',15),xlabel('number of iterates','FontSize',10);